...
![Functional circuit](./img/Ima_Ctrl_circuit.png) **Figure 3B** Functional submodel extracted from *SignalingProfiler* 2.0 output linking BCR-ABL to cellular phenotypes modulated upon imatinib treatment. Highlighted proteins are validated by western blot analysis.
...
all_proteins_df %>% dplyr::select(gene_name, final_score.ima, final_score.res) -> sub_df

sum(rowSums(is.na(sub_df)) == 1) + nrow(sub_df %>% filter(final_score.ima * final_score.res < 0))
...
dr_nodes <- c("BCL2" , "TNFRSF17" , "BCR_ABL" , "FLT4" , "KDR" , "FLT1" , 
              ...
              "CD79B" , "CDA" , "PDGFRA" , "PRKCA" , "AMPK" , "CDK1" , "SYK" , "CRBN" , "DNA_DAMAGE" , "RNMT" , "EZH2" , "AXL" , "ALK" , "IDH1" , "IDH2" ,
...
as_data_frame(drugg_net_clean, what = 'vertices')
...
ggplot(score_df_ordered, aes(x = gene_name1, y = drug_score,
  ...
  ggrepel::geom_text_repel(max.overlaps = 100) -> plot_new
```
...
obs_ic50 <- read_tsv('../results/res_sens/druggability_score/data_validation_druggability copy.tsv', show_col_types = FALSE)
obs_ic50 %>% mutate_at(c('IC50_Sens', 'IC50_Res'), log2) %>% 
  mutate(IC50_Res = IC50_Res*-1, IC50_Sens = IC50_Sens*-1) -> obs_ic50_drugg

obs_ic50_drugg[is.na(obs_ic50_drugg)] <- 0
...
inner_join(score_df %>%
             dplyr::select(gene_name, drug_score, drug_score_norm),
           obs_ic50_drugg, by = 'gene_name') -> resulting_df
...
long_df <- long_df %>% mutate(class_drugg = ifelse(drug_score < 0, 
                                             'Negative Druggability Score', 
                                            'Positive Druggability Score')) 

long_df$class_drugg <- factor(long_df$class_drugg, levels = c(  'Negative Druggability Score', 
                                                               'Positive Druggability Score'))
...
ggplot(long_df,
       aes(x = label, y = value, 
           fill = name)) +
  facet_grid(~class_drugg,scales = 'free_x', space = 'free_x')+
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), width = 0.9) +
  xlab('')+
  ylab('-log2(IC50)') +
  theme_classic() +
  scale_fill_manual(values = c('#CD1C67', '#97BDE4')) +
  theme(text = element_text(size = 10),
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5),
        legend.position = 'bottom', 
        legend.title = element_blank()) #-> barplot_IC50

ggsave('../../graphs/scatterplot_ic50.pdf', barplot_IC50, width = 6, height = 4, dpi = 300, device = 'pdf')

# Fold change
resulting_df %>% mutate(FC_IC50 = IC50_Res - IC50_Sens) -> resulting_fc_df

resulting_fc_df %>%
  pivot_longer(cols = c('FC_IC50')) -> long_df
...
ggplot(long_df,
  ...
  theme(text = element_text(size = 10),
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5),
        legend.position = 'bottom', 
        legend.title = element_blank()) -> fold_change_plot

ggsave('../../graphs/scatterplot_ic50_fold_change.pdf', fold_change_plot, width = 6, height = 4, dpi = 300, device = 'pdf')
```
